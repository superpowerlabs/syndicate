...
describe("Integration test", function () {
  ...
  describe('integrations test', async function () {
    ...
    it("testing that rewards after lock time has same weight", async function () {

      await sSynr.connect(superAdmin).updateRole(swapper.address, await sSynr.ROLE_TOKEN_DESTROYER());
      await synr.connect(deployer).updateRole(swapper.address, await synr.ROLE_TOKEN_CREATOR());

      const weeks16 = 16 * 7 * 24 * 3600 + 3600;
      ...
      await corePool.connect(user1).stake(normalize(1000),
          (await ethers.provider.getBlock()).timestamp + weeks16, true);

      await increaseBlockTimestampBy(weeks16);
      const balanceAfter16Weeks = await corePool.pendingYieldRewards(user1.address)

      await increaseBlockTimestampBy(weeks16);

      const balanceAfter32Weeks = await corePool.pendingYieldRewards(user1.address)

      expect(balanceAfter32Weeks.div(200000000)).equal(balanceAfter16Weeks.div(100000000))
